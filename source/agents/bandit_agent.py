...
class BanditAgent(Agent):
  ...
  def take_action(self, observation: int, prev_reward: int, action_mask: Optional[np.array] = None) -> int:
    ...
    if action_mask is not None:
      masked_action_values = np.ma.masked_array(self._action_values, 1 - action_mask)
    ...
    if self._prev_action is not None:
      ...
      self._action_values[self._prev_action] += self._learning_rate * (prev_reward - prev_action_value)

    self._prev_action = action
    return action

  def play_episode(self, env: Env, learning: bool = True, epsilon: float = 0.0) -> Tuple[float, int]:
...
class ContextualBanditAgent(Agent):
  ...
  def update_action_values(self, prev_reward: float):
    # update action values: incremental implementation
      ...
      self._action_values[self._prev_observation][self._prev_action] += self._learning_rate * (prev_reward - prev_action_value)
  ...
  def take_action(self, observation: int, prev_reward: int, action_mask: Optional[np.array] = None) -> int:
    ...
    if random.random() < self._epsilon:
      ...
      if action_mask is not None:
        masked_action_values = np.ma.masked_array(masked_action_values, 1 - action_mask)
...
def test_bandit_agent():
  agent = BanditAgent(observation_space=Discrete(4), action_space=Discrete(5), learning_rate=0.5, epsilon=0, training=True, initial_value=5.0)
...
def test_contexual_bandit_agent():
  agent = ContextualBanditAgent(observation_space=Discrete(4), action_space=Discrete(5), learning_rate=0.5, epsilon=0, training=True, initial_value=5.0)
